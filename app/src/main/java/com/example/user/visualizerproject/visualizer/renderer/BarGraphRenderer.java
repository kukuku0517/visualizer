 ...
 */
package com.example.user.visualizerproject.visualizer.renderer;
...
//        mDivisions = 1;
        for (int i = 0; i < data.floats.length; i++) {
            dctPoints[i * 4] = i * 4 * mDivisions;
            dctPoints[i * 4 + 2] = i * 4 * mDivisions;


            dctPoints[i * 4 + 1] = 0;
            dctPoints[i * 4 + 3] = (float) data.floats[i]*10;

        }

        canvas.drawLines(dctPoints, mPaint);
    }
}
